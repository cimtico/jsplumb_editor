...
jsPlumbUtil = {
    ...
    group : function(g) {
        if (jsPlumbUtil.logEnabled && typeof console != "undefined") console.group(g);
    },
    groupEnd : function(g) {
        if (jsPlumbUtil.logEnabled && typeof console != "undefined") console.groupEnd(g);
    },
    time : function(t) {
        if (jsPlumbUtil.logEnabled && typeof console != "undefined") console.time(t);
    },
    timeEnd : function(t) {
        if (jsPlumbUtil.logEnabled && typeof console != "undefined") console.timeEnd(t);
    }
};/*
 * jsPlumb
 * 
...
_getAttribute = function(el, attName) {
...
_hasClass = function(el, clazz) {
    return jsPlumb.CurrentLibrary.hasClass(_getElementObject(el), clazz);
},
...
overlayCapableJsPlumbUIComponent = function(params) {
    ...
    var processOverlay = function(o) {
        ...
        if (_isArray(o)) {    // this is for the shorthand ["Arrow", { width:50 }] syntax
            ...
            _newOverlay = new jsPlumb.Overlays[self._jsPlumb.getRenderMode()][o]({
                component:self, 
                _jsPlumb:self._jsPlumb
                });
        } else {
    ...
    */
    ...
    this.makeAnchor = function() {
        ...
        // is it the name of an anchor type?
            ...
            newAnchor = jsPlumb.Anchors[arguments[0]]({
                elementId:elementId, 
    ...
    this.makeTarget = function(el, params, referenceParams) {                        
        ...
        _doOne = function(_el) {
            ...
            _drop = function() {
                ...
                targetCount = _currentInstance.select({
                    target:elid
                }).length;
    ...
    this.makeSource = function(el, params, referenceParams) {
        ...
        _doOne = function(_el) {
            ...
            var mouseDownListener = function(e) {
                ...
                var myOffsetInfo = _updateOffset({
                    elId:elid
                });        
    ...
    */
    ...
    var adjustForParentOffsetAndScroll = function(xy, el) {
        ...
        if (offsetParent != null) {
            var po = offsetParent.tagName.toLowerCase() === "body" ? {
                left:0,
                top:0
            } : _getOffset(offsetParent),
            so = offsetParent.tagName.toLowerCase() === "body" ? {
                left:0,
                top:0
            } : {
    ...
    */
    ...
    AnchorManager = function() {
        ...
        var _updateAnchorList = function(lists, theta, order, conn, aBoolean, otherElId, idx, reverse, edgeId, elId, connsToPaint, endpointsToPaint) {
            ...
            if (listToRemoveFrom) {
                var rIdx = _findWithFunction(listToRemoveFrom, function(e) {
                ...
                if (rIdx != -1) {
                    ...
                    for (var i = 0; i < listToRemoveFrom.length; i++) {
                        _addWithFunction(connsToPaint, listToRemoveFrom[i][1], function(c) {
                            return c.id == listToRemoveFrom[i][1].id
                        });
                        _addWithFunction(endpointsToPaint, listToRemoveFrom[i][1].endpoints[idx], function(e) {
            ...
            for (var i = 0; i < listToAddTo.length; i++) {
                ...
                _addWithFunction(connsToPaint, listToAddTo[i][1], function(c) {
                    return c.id == listToAddTo[i][1].id
                });                
                _addWithFunction(endpointsToPaint, listToAddTo[i][1].endpoints[idx], function(e) {
        ...
        this.redraw = function(elementId, ui, timestamp, offsetToUI) {
            ...
            for (var i = 0; i < endpointConnections.length; i++) {
                ...
                if (sourceContinuous || targetContinuous) {
                    ...
                    if (elementId != targetId) _updateOffset( {
                        elId : targetId, 
                        timestamp : timestamp
                    }); 
                    if (elementId != sourceId) _updateOffset( {
                        elId : sourceId, 
                        timestamp : timestamp
                    }); 
                    ...
                    if (sourceContinuous) _addWithFunction(anchorsToUpdate, sourceId, function(a) {
                        return a === sourceId;
                    });
                    if (targetContinuous) _addWithFunction(anchorsToUpdate, targetId, function(a) {
                        return a === targetId;
                    });
                    _addWithFunction(connectionsToPaint, conn, function(c) {
                        return c.id == conn.id;
                    });
                    if ((sourceContinuous && oIdx == 0) || (targetContinuous && oIdx == 1))
                        _addWithFunction(endpointsToPaint, conn.endpoints[oIdx], function(e) {
            ...
            for (var i = 0; i < endpointConnections.length; i++) {
                ...
                if (otherEndpoint.anchor.constructor == DynamicAnchor) {                                         
                    ...
                    _addWithFunction(connectionsToPaint, endpointConnections[i][0], function(c) {
                        return c.id == endpointConnections[i][0].id;
                    });
                    // all the connections for the other endpoint now need to be repainted
                    for (var k = 0; k < otherEndpoint.connections.length; k++) {
                        if (otherEndpoint.connections[k] !== endpointConnections[i][0])                            
                            _addWithFunction(connectionsToPaint, otherEndpoint.connections[k], function(c) {
    ...
    var Connection = function(params) {
        ...
        */
        ...
        this.paint = function(params) {
            ...
            var sourceInfo = _updateOffset( {
                elId : elId, 
                offset : ui, 
                recalc : recalc, 
                timestamp : timestamp
            }),
            targetInfo = _updateOffset( {
                elId : tId, 
                timestamp : timestamp
            }); // update the target if this is a forced repaint. otherwise, only the source has been moved.
    ...
    var Endpoint = function(params) {
        ...
        */
        ...
        this.areConnectionsBidirectional = function() {
            return _connectionsBidirectional;
        };
        this.setConnectionsBidirectional = function(b) {
            _connectionsBidirectional = b;
        };
        ...
        this.detach = function(connection, ignoreTarget, forceDetach, fireEvent, originalEvent) {
            var idx = _findWithFunction(self.connections, function(c) {
        ...
        this.setElement = function(el, container) {
            ...
            var newParentElement = _getParentFromParams({
                source:parentId, 
                container:container
            }),
        ...
        if (jsPlumb.CurrentLibrary.isDragSupported(_element)) {
            ...
            var start = function() {    
                ...
                jsPlumb.CurrentLibrary.setOffset(placeholderInfo.element, {
                    left:po[0], 
                    top:po[1]
                    });                                                            
    ...
    jsPlumb.Connectors.Bezier = function(params) {
        ...
        this.compute = function(sourcePos, targetPos, sourceEndpoint, targetEndpoint, sourceAnchor, targetAnchor, lineWidth, minWidth) {
            ...
            if (minWidth && _h < minWidth) {
                ...
                _sy = _sy + posAdjust ;
                _ty = _ty + posAdjust;
                _CP[1] =  _CP[1] + posAdjust;
                _CP2[1] = _CP2[1] + posAdjust;
            }
        ...
        this.draw = function(component, currentConnectionPaintStyle, componentDimensions) {
            ...
            if (td.width !=  null) {
                ...
                if (component.pointOnPath) {
                    ...
                    cxy = {
                        x:locToUse[0] * componentDimensions[2],
...
(function() {
    ...
    _applyStyles = function(node, style, component, _jsPlumb) {
        ...
        if(style["dashstyle"]) {
            if (component.strokeNode == null) {
                component.strokeNode = _node("stroke", [0,0,0,0], {
    ...
    VmlComponent = function() {                
        ...
        this.initOpacityNodes = function(vml) {
            self.opacityNodes["stroke"] = _node("stroke", [0,0,1,1], {
                opacity:"0.0"
            }, vml, self._jsPlumb);
            self.opacityNodes["fill"] = _node("fill", [0,0,1,1], {
                opacity:"0.0"
            }, vml, self._jsPlumb);                            
        };
...
(function() {
    ...
    _pageXY = function(el) {
        return jsPlumb.CurrentLibrary.getPageXY(el);
    },
    _clientXY = function(el) {
        return jsPlumb.CurrentLibrary.getClientXY(el);
    };
...
},w=Math.pow(2,-65),u=function(a,b){
    for(var f=[],d=b.length-1,h=2*d-1,g=[],c=[],k=[],i=[],l=[[1,0.6,0.3,0.1],[0.4,0.6,0.6,0.4],[0.1,0.3,0.6,1]],e=0;e<=d;e++)g[e]=m.subtract(b[e],a);
    for(e=0;e<=d-1;e++){
        c[e]=
        m.subtract(b[e+1],b[e]);
        c[e]=m.scale(c[e],3)
        }
        for(e=0;e<=d-1;e++)for(var n=0;n<=d;n++){
        k[e]||(k[e]=[]);
        k[e][n]=m.dotProduct(c[e],g[n])
        }
        for(e=0;e<=h;e++){
        i[e]||(i[e]=[]);
        i[e].y=0;
        i[e].x=parseFloat(e)/h
        }
        h=d-1;
    for(g=0;g<=d+h;g++){
        e=Math.max(0,g-h);
        for(c=Math.min(g,d);e<=c;e++){
            j=g-e;
            i[e+j].y=i[e+j].y+k[j][e]*l[j][e]
            }
        }
        d=b.length-1;
i=r(i,2*d-1,f,0);
...
k=m.square(h);
for(e=l=0;e<i;e++){
    h=m.subtract(a,s(b,d,f[e],null,null));
    h=m.square(h);
    if(h<k){
        k=h;
        l=f[e]
...
if(h<k){
    ...
    for(var n=1;n<=b;n++){
        ...
        for(o=1;o<b;o++){
            var m=i*a[o].x+e*a[o].y+n;
            m>l?l=m:m<max_distance_below&&(max_distance_below=m)
            }
            o=e;
        l=(1*(n-l)-o*0)*(1/(0*o-i*1));
            o=e;
            e=n-max_distance_below;
            i=(1*
            e-o*0)*(1/(0*o-i*1));
            e=Math.min(l,i);
            if(Math.max(l,i)-e<w){
            c=a[b].x-a[0].x;
            k=a[b].y-a[0].y;
            f[0]=0+1*(c*(a[0].y-0)-k*(a[0].x-0))*(1/(c*0-k*1));
            return 1
            }
        }
        s(a,b,0.5,h,g);
a=r(h,b,c,d+1);
b=r(g,b,k,d+1);
for(d=0;d<a;d++)f[d]=c[d];
for(d=0;d<b;d++)f[d+a]=k[d];
return a+b
},s=function(a,b,f,d,h){
    for(var g=[[]],c=0;c<=b;c++)g[0][c]=a[c];
    for(a=1;a<=b;a++)for(c=0;c<=b-a;c++){
        g[a]||(g[a]=[]);
        g[a][c]||(g[a][c]={});
        g[a][c].x=(1-f)*g[a-1][c].x+f*g[a-1][c+1].x;
        g[a][c].y=(1-f)*g[a-1][c].y+f*g[a-1][c+1].y
        }
        if(d!=null)for(c=
        0;c<=b;c++)d[c]=g[c][0];
    if(h!=null)for(c=0;c<=b;c++)h[c]=g[b-c][c];
    return g[b][0]
    },v={},x=function(a){
    var b=v[a];
    if(!b){
    ...
    return function(b){
        for(var c=1,d=0;d<a.length;d++)c=c*a[d](b);
        return c
        }
    };
...
for(var c=1;c<a;c++){
    for(var k=[new f(a)],i=0;i<a-c;i++)k.push(new d);
    for(i=0;i<c;i++)k.push(new h);
    b.push(new g(k))
    }
    b.push(new function(){
    return function(b){
...
return b
    ...
    for(var f=x(a.length-1),d=0,h=0,g=0;g<a.length;g++){
        d=d+a[g].x*f[g](b);
        h=h+a[g].y*f[g](b)
        }
        return{
    ...
    for(var d=p(a,b),h=0,g=f>0?1:-1,c=null;h<Math.abs(f);){
        b=b+0.005*g;
        c=p(a,b);
        h=h+Math.sqrt(Math.pow(c.x-d.x,2)+Math.pow(c.y-d.y,2));
        d=c
        }
        return{
        point:c,
    ...
    var f=p(a,b),d=p(a.slice(0,a.length-1),b),h=d.y-f.y,f=d.x-f.x;
    return h==0?Infinity:
    Math.atan(h/f)
    };
...
window.jsBezier={
    ...
    gradientAtPointAlongCurveFrom:function(a,b,f){
        ...
        return t(a,b.location)
        },
    nearestPointOnCurve:function(a,b){
        var f=u(a,b);
        return{
            point:s(b,b.length-1,f.location,null,null),
            location:f.location
            }
        },
pointOnCurve:p,
pointAlongCurveFrom:function(a,b,f){
    return q(a,b,f).point
    },
